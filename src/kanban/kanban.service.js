...
import { AVAILABLE_STATUSES, getCandidateNameQuery, getFilterStatusRequest } from '../utils/kanban'
...
export const getJobOrders = (bmId, start = 0) =>
  post(
    ...
    {
      fields: 'id,clientContact,clientCorporation,isOpen,owner,status,title,employmentType,description',
...
export const getJobSubmissions = (jobOrderId, start = 0) =>
  post(
    ...
    {
      query: `jobOrder.id:${jobOrderId} AND isDeleted:false AND (${getFilterStatusRequest(AVAILABLE_STATUSES)})`,
      start,
    },
    {
      fields:
...
export const createJobSubmission = (jobSubmission) => put('entity/JobSubmission', jobSubmission)
...
export const deleteJobSubmission = (jobSubmissionId) => del(`entity/JobSubmission/${jobSubmissionId}`)
