...
import { flatten, is, isEmpty, isNil, path, pathOr, prop, propOr, union } from 'ramda'

import { Hourglass, HourglassFull } from '../components/svgs'

import { DIFF_5_DAYS, DIFF_10_DAYS, isOverDiff } from './date'
...
export const RECRUITMENT_STATUSES = [STATUS_ITV1, STATUS_ITV2, STATUS_OFFER, STATUS_WF_FEEDBACK, STATUS_NO_GO]

export const getFilterStatusRequest = (statuses) => statuses.map((status) => `status:"${status}"`).join(' OR ')
...
export const createItemFilter = (item) => [`name:${item}*`, `firstName:${item}*`, `lastName:${item}*`]
...
export const getCandidateNameQuery = (query) => {
  ...
  const queries = prop('length', nameParts) > 1 ? queryParts.concat(namePart) : queryParts
  return queries.join(' OR ')
}

export const unionArrays = (l, r) => (is(Array, l) && is(Array, r) ? union(l, r) : r)
