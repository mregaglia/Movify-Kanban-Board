...
export const getCode = (url) => (hasCode(url) ? url.split('code=')[1].split('&')[0] : '')

export const getAuthorizeUrl = () =>
  `${AUTH_URL}authorize${formatQueryParams({
...
export const getAccessToken = (code) =>
  fetch(
    `${AUTH_URL}token${formatQueryParams({
...
export const login = (accessToken) =>
  fetch(
    `${process.env.REACT_APP_BASE_URL}login${formatQueryParams({
      version: '*',
...
export const refreshToken = (refreshTokenProp) =>
  fetch(
    `${AUTH_URL}token${formatQueryParams({
      grant_type: 'refresh_token',
      refresh_token: refreshTokenProp,
      client_id: process.env.REACT_APP_BH_CLIENT_ID,
      client_secret: process.env.REACT_APP_BH_CLIENT_SECRET,
    })}`,
    { method: 'POST' }
