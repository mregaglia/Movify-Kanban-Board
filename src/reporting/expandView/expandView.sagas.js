...
export const getExpandedViewDataCvSent = (state) =>
  state.kpi.dataEmployee.datasBusinessManager.CV_SENT_EXPANDED_VIEW;
...
export function* getDetailDataSaga(action) {
  ...
  let clientOrCandidate = pathOr(
    "",
    ["payload", "CLIENT_OR_CANDIDATE"],
    action
  );
  ...
  let lastName =
    type === NEW_VACANCY
      ? pathOr(
          "",
          ["payload", "DATA", "clientContact", "lastName"],
          action
        ).trim()
      : type === CV_SENT
      ? action?.payload?.DATA?.candidate?.lastName ?? ""
      : pathOr("", ["payload", "DATA", "data", 0, "lastName"], action).trim();
  let firstName =
    type === NEW_VACANCY
      ? pathOr(
          "",
          ["payload", "DATA", "clientContact", "firstName"],
          action
        ).trim()
      : type === CV_SENT
      ? action?.payload?.DATA?.candidate?.firstName ?? ""
      : pathOr("", ["payload", "DATA", "data", 0, "firstName"], action).trim();
  ...
  try {
    ...
    if (clientOrCandidate === IS_CLIENT) {
      ...
      if (type === NEW_VACANCY) {
        clientCorporationName = pathOr(
          "",
          ["payload", "DATA", "clientCorporation", "name"],
          action
        ).trim();
      ...
      if (type === INTAKES) {
        ...
        clientCorporationName = yield call(
          getCompanyNameByClientContactId,
          clientId
        );
        details = { ...details, COMPANY: clientCorporationName };
