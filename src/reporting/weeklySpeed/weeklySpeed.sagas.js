...
import { getCandidateCategory, getNoteProspectionLastYear } from './weeklySpeek.service'
...
export const getInterviewScheduled = (state, weekLabel) =>
  state.kpi.dataEmployee.datasRecruitment.INTERVIEW_SCHEDULED[weekLabel]
...
export function* getGaugeLimit() {
  try {
    const occupation = yield select(getOccupationFromEmployeeSelected)
...
export function* calculateWeeklySpeedForRecruitment(categories, weekLabel, occupation) {
  ...
  try {
      ...
      for (let i = 0; i < categories.length; i += 1) {
        if (categories[i].length > 0) {
          for (const key of sourcingOfficerPoints.keys()) {
            ...
            if (
              (key === firstCategoryId && [ID_IOS_DEVELOPER, ID_BUSINESS_MANAGER].includes(key)) ||
              (allCategoryIds.includes(key) && key === ID_ANDROID_DEVELOPER)
            ) {
              const { points } = sourcingOfficerPoints.get(key)
              weeklySpeed += points
        ...
        if (!isAlreadyCounted) weeklySpeed += 1
    ...
    const numberOfInterviewScheduled = yield select(getInterviewScheduled, weekLabel)
    if (occupation.includes(TALENT_ACQUISITION)) {
      const numberOfInterviewDone = yield call(getInterviewDoneSaga, weekLabel)
...
export function* getCandidatesCategory(idsCandidate) {
  ...
  try {
    while (idsCandidate.length > maxCall) {
      const [
        candidateCategoryOne,
        candidateCategoryTwo,
        candidateCategoryThree,
        candidateCategoryFour,
        candidateCategoryFive,
      ] = yield all([
    ...
    if (idsCandidate.length > 0) {
      for (let i = 0; i < idsCandidate.length; i += 1) {
        const result = yield call(getCandidateCategory, idsCandidate[i])
...
export function* calculateWeeklySpeedForBusinessManager(
  idEmployee,
  dateEnd,
  prospectionMeetingsDoneFromLastWeek,
  weekLabel
) {
  ...
  try {
    ...
    prospectionMeetingsDoneFromTheBeginning = prospectionMeetingsDoneFromTheBeginning?.filter(
      (single) => single.action === 'Prospection'
    )
    ...
    prospectionMeetingsDoneFromLastWeek.forEach((currentMeeting) => {
      if (currentMeeting?.clientContacts?.data?.length > 0) {
        ...
        weeklySpeedWithoutCvSent += hasAlreadyBeenContacted
          ? POINT_PROSPECTION_MEETING_DONE_RE_PROSPECTION
          : POINT_PROSPECTION_MEETING_DONE_NEW_CONTACT
...
export function* calculateAllWeeklySpeedForBusinessManager(idEmployee, dates, prospectionDone) {
  try {
    yield all([
      yield call(
        calculateWeeklySpeedForBusinessManager,
        idEmployee,
        dates[0].start,
        prospectionDone.FIRST_WEEK,
        FIRST_WEEK
      ),
      yield call(
        calculateWeeklySpeedForBusinessManager,
...
export default function weeklySpeedSagas() {
  return [takeLatest(GET_GAUGE_LIMIT, getGaugeLimit)]
}
